"""Head tracking and ellipse masks (the v6 logic validated on 2026-09-22/23).
...
def observe(person):
    ...
    o = dict(center=np.mean(have, axis=0), fcenter=np.mean(face, axis=0) if face else None, scale=max(est),
             vis=vis, hconf=max(conf(n) for n in HEAD), back=vis < .15, angle=None)
...
def track(people_per_frame, fps):
...
def build_masks(people_per_frame, fps, mode="smart"):
    ...
    tracks = [t for t in track(people_per_frame, fps) if len(t) >= max(4, fps * .2) and max(o["hconf"] for _, o in t) >= .5]
    ...
    for tid, t in enumerate(tracks):
        ...
        if on.sum() < max(4, fps * .2):
            continue
        cx, cy, sc, an = (np.full(L, np.nan) for _ in range(4))
        for i, n in enumerate(range(n0, n1 + 1)):
            o = idx.get(n)
            if o is None:
                continue
            c = o["fcenter"] if (o["fcenter"] is not None and o["vis"] >= .3) else (o["center"] if mode != "face" else None)
            if c is not None:
                cx[i], cy[i] = c
            sc[i] = o["scale"]
        ...
        if ok.sum() < 3:
            continue
        cx, cy = _smooth(_fill_nan(cx), w5), _smooth(_fill_nan(cy), w5)
        ...
        for i in range(L):
            if not draw[i]:
                continue
            s = sc[i]
            a, b = .80 * s, .92 * s          # generous: roughly 1.6x head width
            ccx, ccy = cx[i], cy[i] + .12 * s
